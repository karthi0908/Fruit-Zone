            ...
            if (!existingBtn) {
                ...
                try {
                    ...
                    if (balRes.ok) {
                        ...
                        previousBalance = parseFloat(balData.previousBalance) || 0;
                    }
                } catch (e) {
                    console.error('Could not fetch previous balance', e);
                }
                ...
                try {
                    ...
                    if (empRes.ok) {
                        ...
                        previousEmpty = parseInt(empData.previousEmptyBalance, 10) || 0;
                    }
                } catch (e) {
                    console.error('Could not fetch previous empty balance', e);
                }
...
function renderTable(data = []) {
    ...
    data.forEach((record, index) => {
        ...
        const f = (val) => (val === undefined || val === null || val === '') ? '' : val;
...
async function enrichRecordsWithPreviousBalances(data, date) {
    if (!Array.isArray(data)) return data;

    for (let record of data) {
        if (!record.shopName) continue;

        if (record.oldBalance === null || record.oldBalance === undefined) {
            try {
                const balRes = await fetch(`${SERVER_URL}/previousBalance?date=${date}&shopName=${encodeURIComponent(record.shopName)}`);
                if (balRes.ok) {
                    const balData = await balRes.json();
                    record.oldBalance = parseFloat(balData.previousBalance) || 0;
                }
            } catch (e) {
                record.oldBalance = 0;
            }
        ...
        if (record.oldEmpty === null || record.oldEmpty === undefined) {
            try {
                const empRes = await fetch(`${SERVER_URL}/previousEmptyBalance?date=${date}&shopName=${encodeURIComponent(record.shopName)}`);
                if (empRes.ok) {
                    const empData = await empRes.json();
                    record.oldEmpty = parseInt(empData.previousEmptyBalance, 10) || 0;
                }
            } catch (e) {
                record.oldEmpty = 0;
            }
        ...
        // Recalculate balance and balanceEmpty
        const tot = parseFloat(record.totalAmount) || 0;
        const giv = parseFloat(record.givenAmount) || 0;
        const oldB = parseFloat(record.oldBalance) || 0;
        record.balance = tot + oldB - giv;

        const emp = parseInt(record.empty, 10) || 0;
        const ret = parseInt(record.emptyReturn, 10) || 0;
        const oldE = parseInt(record.oldEmpty, 10) || 0;
        record.balanceEmpty = emp + oldE - ret;
    }
    return data;
}
...
async function fetchRecordsForDate() {
    ...
    try {
        const response = await fetch(`${SERVER_URL}/date?date=${date}&_t=${new Date().getTime()}`, { cache: 'no-store' });
        if (response.ok) {
            let data = await response.json();
            data = await enrichRecordsWithPreviousBalances(data, date);
...
function getRowData(index, shopName) {
    ...
    inputs.forEach(inp => {
        ...
        const isFloatCol = (col === 'totalAmount' || col === 'balance' || col === 'oldBalance' || col === 'givenAmount' || col.endsWith('Amount') || col.endsWith('Rate'));
        baseData[col] = isFloatCol ? val : parseInt(val, 10);
    });
